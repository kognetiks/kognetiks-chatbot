 ...
 * It allows users to configure the settings links and other parameters
 * 
 *
...
function chatbot_chatgpt_deactivation_feedback() {
    ...
    chatbot_chatgpt_back_trace("Email Ready");
    ...
    if (wp_mail($to, $subject, strip_tags($body), $headers)) {
        ...
        chatbot_chatgpt_back_trace("Email Sent Successfully");
    } else {
        // DIAG - Log the email failed status
        chatbot_chatgpt_back_trace("Email Failed to Send");
    }

    // DIAG - Log the email body
    chatbot_chatgpt_back_trace("Email Done");
