...
= 2.0.1 =
* Supports OpenAI's latest model gpt-4o and gpt-4o-2024-05-13
...
Max Prompt Tokens
Controls the maximum prompt token usage: limits apply to the total number of tokens used in all completions thought the Run's lifecycle.
If the Max Prompt Tokens is set to 500, then prompts will be truncated at 500 tokens.

Max Completion Tokens
Controls the maximum completion token usage: limits apply to the total number of tokens used in all completions thought the Run's lifecycle.
If the Max Completion Tokens is set to 1000, then completion will cap the output at 1000 tokens.

https://...
Temperature
Controls randomness: Lowering results in the less random completions. As the temperature approarches zero, the model will become determinisc and repetitive.

Top P
Control diversity via nucleus sampling: 0.5 means have of all likelihood-weighted options are considered.
